...
/* =====================================================================
   NET_MasterQuery  —  client-side master server query with key security
   Ported from FWGS/xash3d-fwgs and adapted for tyabus/xash3d APIs.
   ===================================================================== */

#define MS_SCAN_HEADER   "1\xFF" "0.0.0.0:0\0"
#define MS_SCAN_HDR_LEN  ( sizeof( MS_SCAN_HEADER ) - 1 )

/* GoldSrc A2S_INFO query payload */
static const byte A2S_GOLDSRC_INFO_PAYLOAD[] = "\xFF\xFF\xFF\xFFTSource Engine Query\0";
#define A2S_GOLDSRC_INFO_LEN  ( sizeof( A2S_GOLDSRC_INFO_PAYLOAD ) )
...
CLIENT-SIDE.
Sends a scan request to all Xash3D masters (with key + NAT flag)
and a GoldSrc scan request to any GoldSrc-flagged masters.

Parameters:
  key    — random uint32 stored in cls.internetservers_key,
            the Xash3D master server echoes it in the reply header
            so we can drop spoofed / stale packets.
  nat    — nonzero → ask for NAT-compatible servers
  filter — gamedir filter string (may be NULL or empty)

Returns true when at least one DNS resolve is still pending.
========================
*/
qboolean NET_MasterQuery( uint32_t key, qboolean nat, const char *filter )
{
	...
	/* -- Xash3D protocol request (includes key, clver, etc.) -- */
	Q_memcpy( buf, MS_SCAN_HEADER, MS_SCAN_HDR_LEN );
	info      = buf + MS_SCAN_HDR_LEN;
	remaining = sizeof( buf ) - MS_SCAN_HDR_LEN;
	*info     = '\0';

	if( filter && *filter )
		Info_SetValueForKey( info, "gamedir", filter, remaining );
	else if( GI && GI->gamefolder[0] )
	...
	Info_SetValueForKey( info, "key",      va( "%x", key ),            remaining );

	len  = MS_SCAN_HDR_LEN + Q_strlen( info );
	wait = NET_SendToMasters( NS_CLIENT, len, buf );

	/* -- GoldSrc protocol request (no key, no extensions) --
	   Reuse the same master list; GoldSrc masters historically
	   share the same port (27010) and respond to both formats. */
	Q_memcpy( buf, MS_SCAN_HEADER, MS_SCAN_HDR_LEN );
	info      = buf + MS_SCAN_HDR_LEN;
	remaining = sizeof( buf ) - MS_SCAN_HDR_LEN;
	*info     = '\0';
	...
	MsgDev( D_NOTE, "NET_MasterQuery: key=0x%x nat=%d filter=%s\n",
		key, nat, filter ? filter : "(none)" );

	return wait;
}

/* Exported for netchan.h */
